...
export const signInHitRects = {
  ...
  passwordInput: { left: 0.24, top: 0.540, width: 0.52, height: 0.055 },
...
export function resolveSignInArtboardIntrinsic(): { width: number; height: number } {
  const resolveFn =
    typeof Image.resolveAssetSource === "function"
      ? (Image.resolveAssetSource as (src: ImageSourcePropType) => { width?: number; height?: number })
      : undefined;
...
export function normRectToStyle(rect: NormRect, artboardWidth: number, artboardHeight: number): ViewStyle {
  return {
    position: "absolute",
