#!/usr/bin/env node
/**
 * Runs Firestore rules tests via the emulator.
...
if (!javaHome) {
  console.error(
    "Firestore rules tests require Java (Firebase emulator).\n" +
      "Install: brew install openjdk@21\n" +
      "Then either export JAVA_HOME or re-run npm run test:firestore-rules (auto-detects Homebrew JDK).",
  );
  process.exit(1);
}
...
const cmd =
  'firebase emulators:exec --only firestore --project into-the-pond-rules-test "node --import tsx --test tests/firestore.rules.test.ts"';
