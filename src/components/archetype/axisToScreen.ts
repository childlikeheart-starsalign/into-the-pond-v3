 ...
 */
export function axisToScreenY(axisB: number, insetTop: number, plotHeight: number): number {
  return insetTop + plotHeight - (axisB / 100) * plotHeight;
}

export function axisToScreenX(axisA: number, insetLeft: number, plotWidth: number): number {
