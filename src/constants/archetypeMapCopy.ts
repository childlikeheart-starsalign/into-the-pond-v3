...
export const CENTER_BLEND_CAPTION =
  "You're seeing a mix of a few patterns right now." as const;
 ...
 */
export function buildMapCaption(
  axisA: number,
  axisB: number,
  trail?: MapTrailPoint[],
): string {
  ...
  if (priors.length >= 1) {
    ...
    if (
      !currentIsBlendOrCenter &&
      !priorIsBlendOrCenter &&
      prior.nearest !== current.nearest
    ) {
