...
import { optionalAudioSources } from "@/src/services/audio/optionalAudioSources";

const VOLUME = 0.07;
const FADE_MS = 450;

type UseGateAmbientSoundOptions = {
  active: boolean;
...
/** Quiet evening-pond loop on signed-out cold gate — fades out on key tap. */
export function useGateAmbientSound({ active, reduceMotion = false }: UseGateAmbientSoundOptions) {
  const soundRef = useRef<Audio.Sound | null>(null);
  const loadingRef = useRef(false);
  const fadeMs = reduceMotion ? 0 : FADE_MS;
  const source = optionalAudioSources.gateAmbient;
