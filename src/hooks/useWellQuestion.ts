...
export function useWellQuestion(enabled: boolean): UseWellQuestionResult {
  ...
  const load = useCallback(async () => {
    ...
    try {
      await ensureWellState(uid);
      const response = await getOrAssignTodaysWellQuestion(uid, localDate);
      if (!response.success) {
      ...
      applyQuestionLoadResult(response, uid, localDate, setters);
    } catch (err) {
      ...
      if (__DEV__ && isFunctionsUnavailable(err)) {
        console.warn(
          "[Well] Cloud Functions not found — using local dev preview. Deploy ensureWellState + getOrAssignTodaysQuestion to asia-east2.",
          err,
        );
        ...
        applyQuestionLoadResult(devResponse, uid, localDate, setters);
        return;
      }
  ...
  const reroll = useCallback(async () => {
    ...
    let response;
    try {
      response = await rerollWellQuestion(uid, localDate, requestId);
    } catch (err) {
      if (__DEV__ && isFunctionsUnavailable(err)) {
