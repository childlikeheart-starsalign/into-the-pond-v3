...
async function fallbackRows(uid: string): Promise<LessonAccessRow[]> {
  const lessons = await database
    .get<LocalLesson>("local_lessons")
    .query(Q.sortBy("lesson_order", Q.asc))
    .fetch();
  const completed = await database
    .get<LocalCompletedLesson>("local_completed_lessons")
  ...
  const subscriptionStatus =
    (profile[0]?.subscriptionStatus as SubscriptionStatus | undefined) ?? "free";
...
export async function getLessonRowsForClassroom(uid: string): Promise<LessonAccessRow[]> {
  const accessRows = await database
    .get<LocalLessonAccess>("lesson_access")
    .query(Q.where("uid", uid))
    .fetch();
