...
export function computeEffectiveTier(input: EffectiveTierInput): SubscriptionStatus {
  ...
  const hasTier2 =
    subSet.has(PRODUCT_IDS.tier2Monthly) || lifetimeSet.has(PRODUCT_IDS.tier2Lifetime);
  if (hasTier2) return "fiberglass";

  const hasTier1 =
    subSet.has(PRODUCT_IDS.tier1Monthly) || lifetimeSet.has(PRODUCT_IDS.tier1Lifetime);
